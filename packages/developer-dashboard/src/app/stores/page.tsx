            ...
            {stores.map(store => (
              <tr key={store.id} style={{ borderBottom: '1px solid var(--border)' }}>
                <td style={{ padding: '1rem' }}>
                  ...
                  <div style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>/{store.slug}</div>
                </td>
                ...
                <td style={{ padding: '1rem', textAlign: 'center' }}>
                  <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center' }}>
                    <Link href={`http://localhost:3000/store/${store.slug}`} target="_blank" className="btn btn-ghost btn-sm"><ExternalLink size={14} /> Visit</Link>
                  </div>
                </td>
              </tr>
            ))}
