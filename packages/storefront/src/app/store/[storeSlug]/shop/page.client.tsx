...
export default function StoreShopPage({ initialProducts, initialTotalPages, initialCategories }: {
  initialProducts?: any[];
  ...
  const filtersRef = useDismiss(showFilters, closeFilters);
    ...
    <div style={{ position: 'relative', zIndex: 1, padding: '2rem 0' }}>
      ...
      {showFilters && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }} onClick={closeFilters}>
          <div className="card" ref={filtersRef} tabIndex={-1} style={{ width: '100%', maxWidth: 560, maxHeight: '88vh', display: 'flex', flexDirection: 'column', padding: 0, overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
            {/* Overlay header */}
