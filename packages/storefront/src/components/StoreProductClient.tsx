...
export default function StoreProductClient({ initialProduct, slug, storeSlug }: {
  initialProduct: any;
  slug: string;
  storeSlug: string;
}) {
  ...
  const [mainImg, setMainImg] = useState(initialProduct?.images?.[0] || `https://picsum.photos/seed/${initialProduct?.id}/600/600`);
  const { user } = useAuth();
  ...
  const toggleWishlist = async () => {
    ...
    try {
      if (saved) {
        await storeApi.post('/wishlist/remove', { productId: product.id });
        setSaved(false);
      } else {
        await storeApi.post('/wishlist/add', { productId: product.id });
        setSaved(true);
      }
    } catch { /* ignore */ }
  };
  ...
  useEffect(() => {
    if (!slug) return;
    // Fallback hydration if SSR data is missing (API hiccup on first render).
    if (!initialProduct) {
      setLoading(true);
      storeApi.get(`/products/${slug}`).then((r: any) => {
        setProduct(r.data);
        const imgs = r.data.images?.length > 0 ? r.data.images : [`https://picsum.photos/seed/${r.data.id}/600/600`];
        setMainImg(imgs[0]);
        if (user) {
          storeApi.get('/wishlist').then((w: any) => {
            const lists = Array.isArray(w.data) ? w.data : [];
            setSaved(lists.some((l: any) => l.items?.some((it: any) => it.productId === r.data.id)));
          }).catch(() => {});
        }
      ...
      storeApi.get('/wishlist').then((w: any) => {
        const lists = Array.isArray(w.data) ? w.data : [];
        setSaved(lists.some((l: any) => l.items?.some((it: any) => it.productId === product.id)));
      }).catch(() => {});
    }
  }, [slug, user]);

  if (loading) return <div className="container" style={{ padding: '4rem 1rem' }}>
  ...
  return (
    <div className="container" style={{ padding: '2rem 0', position: 'relative', zIndex: 1 }}>
      <ProductSeo product={{ ...product, storeName: product.storeName || '' }} storeSlug={storeSlug} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
        <div>
          <div style={{ aspectRatio: '1', background: 'var(--bg-secondary)', borderRadius: '0.75rem', overflow: 'hidden', marginBottom: '0.75rem', position: 'relative' }}>
            <Image src={mainImg} alt={product.name} fill priority sizes="(max-width: 768px) 100vw, 600px" style={{ objectFit: 'cover' }} onError={e => { (e.target as HTMLImageElement).src = `https://picsum.photos/seed/${product.id}/600/600`; }} />
          </div>
          <div style={{ display: 'flex', gap: '0.5rem', overflowX: 'auto', paddingBottom: '0.25rem' }}>
            {imgs.map((url: string, i: number) => (
              <Image key={i} src={url} alt="" width={64} height={64} onClick={() => setMainImg(url)} loading={i === 0 ? 'eager' : 'lazy'}
                style={{ borderRadius: '0.5rem', objectFit: 'cover', cursor: 'pointer', flexShrink: 0, border: mainImg === url ? '2px solid var(--primary)' : '2px solid transparent' }} onError={e => { (e.target as HTMLImageElement).src = `https://picsum.photos/seed/${product.id}-${i}/600/600`; }} />
            ))}
          </div>
        </div>
            ...
            {product.related.map((p: any) => (
              <Link key={p.id} href={`/store/${storeSlug}/product/${p.slug}`} className="card" style={{ textDecoration: 'none', color: 'inherit' }}>
                <div style={{ aspectRatio: '1', background: 'var(--bg-secondary)', borderRadius: '0.5rem', overflow: 'hidden', marginBottom: '0.75rem', position: 'relative' }}>
                  <Image src={p.images?.[0] || `https://picsum.photos/seed/${p.id}/300/300`} alt={p.name} fill sizes="(max-width: 768px) 50vw, 300px" style={{ objectFit: 'cover' }} loading="lazy" onError={e => { (e.target as HTMLImageElement).src = `https://picsum.photos/seed/${p.id}/300/300`; }} />
                </div>
